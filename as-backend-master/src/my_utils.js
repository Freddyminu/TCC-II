...
// PostgreSQL
function search_article_metadata(res, user_query, count, author, venue, begin_date, end_date){ //o do postgreSQL [FRED]
    ...
    for (let i in q){
        where_search_field.push(`(ar.title ILIKE '%${q[i]}%' OR ar.abstract ILIKE '%${q[i]}%' OR kw.keyword ILIKE '%${q[i]}%')`);
    }
    ...
    // esssa keyword_subquery abaixo foi modificada para funcionar novamente.
        const keyword_subquery = `
    SELECT 
        STRING_AGG(keyword,', ') as KeywordString
    FROM
        keywords kw
    INNER JOIN
...
*/ 
    ...
    \\ esse em cima eh o original!!!!!  

    
    */ 
    

    //console.log(select_query); // tirei o comentario [FRED] 
    ...
    pool.query(select_query, (err, results) => {
        if (err){
            ...
            for (var i = 0; i < results.rowCount; i++){
                ...
                // console.log(results.rows[i]); // eu tirei o comentario [FRED]
